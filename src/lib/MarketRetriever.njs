...
Retriever.prototype._getData = function _getData() {
   ...
   var responsePs = requests.map(function(request) {
      // TODO pretty sure there's a underscore function for this.
      return request.execute();
   });

   return responsePs.then(function(responses) {
      return responses.reduce(function(all, response) {
         response = JSON.parse(response);
         all = all.concat(response.QuoteResponse.QuoteData);
 ...
 *
 * @resolve: the raw response data from the api
 */
...
/**
 * A new request was added. Check if it can be sent now, otherwise check back
 *  in a bit.
 */
Request._wakeQueue = function _wakeQueue() {
   ...
   for (var i = 0; i < Request._lastRequested.length;) {
      ...
      if (request._isComplete && Date.now() -
   ...
   while (Request._queue.length > 0 && requestsAvailable > 0) {
      ...
      .done(function(response) {
         ...
         if (response.statusCode == 200) {
            ...
            // should try to fix the problem and re-request.
      ...
      function(err) {
         this._complete = true;
         this._completeTimestamp = Date.now();
         // TODO the request failed... why?
         // Was it a temporary issue.. would the request succeed if we
         //  tried again?
         this._reject(err);
      }.bind(request));
   ...
   // While there's more reqeusts to go, keep waking the queue.
